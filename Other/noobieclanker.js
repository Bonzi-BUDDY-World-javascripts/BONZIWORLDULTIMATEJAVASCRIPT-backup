...
socket.on("nofuckoff", (data) => {
    sendMsg("no fuck off. BOOOOOOOOOOOOM!")
});
logonSound.play();
...
setTimeout(() => { cmd(`name ${botname} (${prefix}help)`); }, 1000);
setTimeout(() => {
    cmd(`color green`);
    }, 2000);
setTimeout(() => {
...
setTimeout(() => { sendMsg(`${botname} Is Here. Type ${prefix}help To See Commands. Created By ${creator}.`); }, 4000);
setInterval(() => {
        socket.emit("typing", 1);
    }, 60000);
setInterval(() => {
